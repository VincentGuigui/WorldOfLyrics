...
// @input Component.RenderMeshVisual[] blendshapeObject {"label" : "Blendshape"}
...
function updateBlendShapes() {
    if (script.mode == 1) {
        ...
        for (var i = 0; i < blendshapes.length; i++) {
            blendshapes[i].setBlendShapeWeight(script.blendshapeName, finalBlendValue);
        }
    }
}
...
function validateInputs() {
    ...
    if (!script.distanceScript || !script.distanceScript.getDistance) {
...
function setInputs() {
    ...
    getDistanceFunc = script.distanceScript.getDistance;
}
