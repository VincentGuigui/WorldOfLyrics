...
script.getPointsWorldPosition = getPointsWorldPosition;
...
function pinToFaceLandmarks() {
    if (script.placementMode == 0) {
        ...
        var landmarkWorldCenter = camera.screenSpaceToWorldSpace(landmarksCenter, getDepth());
        ...
        for (var j = 0; j < duplicatedObjects.length; j++) {
            pointsWorldPos[j] = camera.screenSpaceToWorldSpace(landmarksPoint[j], getDepth());
...
function validateInputs() {
    ...
    if (!script.headBinding) {
        ...
        cameraObject = script.headBinding.getSceneObject().getParent().getParent();
    }

    if (!cameraObject || !cameraObject.getComponent("Component.Camera")) {
        print("PinToFaceLandmarks, ERROR: Please put Face Landmarks Asset under main Camera object");
        return false;
    } else {
        camera = cameraObject.getComponent("Component.Camera");
    }
